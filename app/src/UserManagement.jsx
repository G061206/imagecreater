...
export function UserManagement() {
  ...
  const visibleUsers = useMemo(() => {
    ...
    return users.filter((item) => `${item.full_name || ""}${item.email}`.toLowerCase().includes(normalized));
  }, [query, users]);
  ...
  return (
    <div className="admin-page">
      ...
      <div className="table-toolbar"><label><MagnifyingGlass size={17} /><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="搜索邮箱或名称" /></label><button>全部角色<CaretDown size={14} /></button></div>
